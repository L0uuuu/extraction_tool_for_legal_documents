"""
LegaTech Tunisia Legal Extractor — v5 (OpenAI GPT-4.1)
=======================================================
Two-phase pipeline:
...
API backend: OpenAI GPT-4.1
  Auth     : OPENAI_API_KEY env var
  Model    : MODEL env var (default: gpt-4.1)

parent_document_id, preceding_article_id, following_article_id
are generated by the model (with Python fallback if omitted).
"""
...
#   AZURE_OPENAI_ENDPOINT=https://modelsetafakna.openai.azure.com/
#   AZURE_OPENAI_DEPLOYMENT=your_deployment_name   (e.g. gpt-4.1)
#   AZURE_OPENAI_API_VERSION=2025-01-01-preview
OPENAI_API_KEY         = os.getenv("AZURE_OPENAI_API_KEY", "")
AZURE_ENDPOINT         = os.getenv("AZURE_OPENAI_ENDPOINT", "https://modelsetafakna.openai.azure.com/")
...
class OpenAICaller:
    """
    Thin wrapper around the OpenAI API (GPT-4.1).
    ...
    def __init__(self):
        if not OPENAI_API_KEY:
            raise RuntimeError(
                ...
                "  Add to your .env:  OPENAI_API_KEY=sk-..."
            )
        ...
        print(f"🔑 OpenAI credentials loaded")
        print(f"🤖 Model: {MODEL}")
    ...
    def call(self, system, user, max_tokens, label=""):
        ...
        for attempt in range(1, RETRY_ATTEMPTS + 1):
            ...
            try:
                response = self._client.chat.completions.create(
                    model=AZURE_DEPLOYMENT,   # ← Azure uses the deployment name here, not the model name
        ...
        raise RuntimeError(f"[{label}] OpenAI call failed after {RETRY_ATTEMPTS} attempts.")
...
-- MODEL-GENERATED DOCUMENT RELATIONS --
  parent_document_id  : Slug identifying the parent document.
                        Format: "tn-{law_type_slug}-{law_number}" where law_type_slug
                        is law_type lowercased with non-alphanumeric chars replaced by "-",
                        and law_number similarly normalised. E.g. "tn-decret-2024-100".
  preceding_article_id: ID of the article immediately before this one in the document.
                        Format: "{parent_document_id}-art-{article_order - 1}"
                        Use "" if this is the first article (article_order == 1).
  following_article_id: ID of the article immediately after this one in the document.
                        Format: "{parent_document_id}-art-{article_order + 1}"
                        Always provide a value (even for the last article).
...
def compute_static_fields(article):
    ...
    # --- article_order (always computed from article_number) ---
    ...
    # --- parent_document_id / preceding_article_id / following_article_id ---
    # Primary: use whatever the model returned.  Fallback: compute from metadata.
    if not article.get("parent_document_id"):
        law_type_slug = re.sub(r'[^a-z0-9]', '-',
                               article.get("law_type", "loi").lower()).strip('-')
        law_num_slug  = re.sub(r'[^a-z0-9]', '-',
                               article.get("law_number", "").lower()).strip('-')
        article["parent_document_id"] = f"tn-{law_type_slug}-{law_num_slug}"
        print(f"   ℹ️  parent_document_id fallback: {article['parent_document_id']}")
    parent_id = article["parent_document_id"]

    if not article.get("preceding_article_id") and article.get("preceding_article_id") != "":
        article["preceding_article_id"] = f"{parent_id}-art-{n-1}" if n > 1 else ""
        print(f"   ℹ️  preceding_article_id fallback applied")

    if not article.get("following_article_id"):
        article["following_article_id"] = f"{parent_id}-art-{n+1}"
        print(f"   ℹ️  following_article_id fallback applied")
...
def run_extraction(input_path, output_path=None, enrich_limit=None,
    ...
    print("\n" + "=" * 60)
    print("🇹🇳 LegaTech Tunisia Extractor v5 — OpenAI GPT-4.1 Edition")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python extractor_openai.py <file> [output.json] "
        ...
        print("  OPENAI_API_KEY   OpenAI API key (sk-...)")
        print("  MODEL            OpenAI model  (default: gpt-4.1)")
        sys.exit(1)
