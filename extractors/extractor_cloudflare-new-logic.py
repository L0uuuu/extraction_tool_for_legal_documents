"""
LegaTech Tunisia Legal Extractor — v4 (Cloudflare Workers AI)
==============================================================
Two-phase pipeline:
...
API backend: Cloudflare Workers AI (OpenAI-compatible)
...
class CFCaller:
    """
    Thin wrapper around the Cloudflare Workers AI OpenAI-compatible endpoint.
    Single key, single model — retries with exponential back-off on failure.
    """
    ...
    def __init__(self):
        if not CF_AUTH_TOKEN:
            raise RuntimeError(
                ...
                "  Add to your .env:  CF_AUTH_TOKEN=your_token\n"
                "                     CF_ACCOUNT_ID=your_account_id"
            )
        if not CF_ACCOUNT_ID:
            raise RuntimeError(
                "CF_ACCOUNT_ID not set.\n"
                "  Add to your .env:  CF_ACCOUNT_ID=your_account_id"
            )
        self._client = OpenAI(
            api_key  = CF_AUTH_TOKEN,
            base_url = f"https://api.cloudflare.com/client/v4/accounts/{CF_ACCOUNT_ID}/ai/v1",
        )
        print(f"🔑 Cloudflare auth token loaded")
    ...
    def call(self, system, user, max_tokens, label=""):
        ...
        for attempt in range(1, RETRY_ATTEMPTS + 1):
            attempt_label = f"{label} [{CF_MODEL} a{attempt}]"
            try:
        ...
        raise RuntimeError(f"[{label}] Cloudflare API call failed after {RETRY_ATTEMPTS} attempts.")
...
def run_extraction(input_path, output_path=None, enrich_limit=None,
    ...
    """
    print("\n" + "=" * 60)
    print("🇹🇳 LegaTech Tunisia Extractor v4 — Cloudflare Edition")
    print("=" * 60)

    if not CF_AUTH_TOKEN or not CF_ACCOUNT_ID:
        raise RuntimeError(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python extractor_cf.py <file> [output.json] "
        ...
        print("  CF_AUTH_TOKEN   Cloudflare API token")
