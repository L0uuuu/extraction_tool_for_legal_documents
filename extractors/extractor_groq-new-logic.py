"""
LegaTech Tunisia Legal Extractor — v4 (Groq Multi-Key / Multi-Model)
=====================================================================
Two-phase pipeline:
...
API backend: Groq
  ...
  Auth     : Bearer token(s) via GROQ_API_KEY_1, GROQ_API_KEY_2, ... env vars
             (also accepts GROQ_API_KEY as a single-key fallback)
  Models   : Tried in order; rotated automatically on rate-limit or failure.

Multi-key / Multi-model rotation strategy
  - Models are tried in priority order (fastest/cheapest first).
  - On 429 (rate-limit) or finish_reason=length the next model is tried.
  - When ALL models on the current key are exhausted, the next API key is used.
  - Exponential back-off is applied between retries on the same model.
"""
...
# ── API keys ──────────────────────────────────────────────────────────────────
# Set any number of keys as GROQ_API_KEY_1, GROQ_API_KEY_2, etc. in your .env
# The script also accepts a bare GROQ_API_KEY (treated as key #1).
...
# ── Model priority list ───────────────────────────────────────────────────────
# Models are tried IN ORDER for every call.
# Put fastest / highest-quota models first.
GROQ_MODELS = [
    "meta-llama/llama-4-scout-17b-16e-instruct",
    "llama-3.3-70b-versatile",   # large, high quality
    "llama3-70b-8192",           # older but stable
    "mixtral-8x7b-32768",        # long context
    "llama3-8b-8192",            # fast, lower quota usage
...
RETRY_ATTEMPTS        = 3         # per-model attempts before rotating
RETRY_BASE_DELAY      = 2         # seconds (doubles on each retry)
ROTATION_WAIT         = 10        # seconds to wait when all models exhausted


# ─────────────────────────────────────────────────────────────────────────────
# GROQ CLIENT / ROTATION STATE
# ─────────────────────────────────────────────────────────────────────────────

class GroqRotator:
    """
    Wraps multiple Groq API keys and models.
    Provides a single call() method that automatically rotates on failure.

    Rotation order:
      For each key -> try models in GROQ_MODELS order
      On 429 or finish_reason=length -> skip to next model
      When all models on a key fail -> skip to next key
      When all keys exhausted -> wait ROTATION_WAIT seconds and restart
    """
    ...
    def __init__(self, api_keys, models):
        if not api_keys:
            raise RuntimeError(
                ...
                "  Set GROQ_API_KEY or GROQ_API_KEY_1, GROQ_API_KEY_2, ... in your .env"
            )
        ...
        print(f"🔑 {len(api_keys)} Groq API key(s) loaded")
    ...
    def _client(self, key):
        if key not in self._clients:
            self._clients[key] = OpenAI(api_key=key, base_url=GROQ_BASE_URL)
    ...
    def _next_model(self):
        """Advance to next model. Returns False when all models exhausted."""
        self._model_idx += 1
    ...
    def _next_key(self):
        """Advance to next key (resets model index). Returns False when all keys tried."""
        self._model_idx = 0
        self._key_idx += 1
    ...
    def call(self, system, user, max_tokens, label=""):
        """
        Call Groq with automatic model/key rotation.
        Returns the assistant message text.
        Raises RuntimeError only when every key+model combination fails.
        """
        total_combos    = len(self.api_keys) * len(self.models)
        attempts_left   = total_combos * RETRY_ATTEMPTS
        tried_this_pass = 0
        ...
        while attempts_left > 0:
            ...
            for attempt in range(1, RETRY_ATTEMPTS + 1):
                attempts_left  -= 1
                tried_this_pass += 1
                attempt_label   = f"{label} [{model} key{key_short} a{attempt}]"
                ...
                try:
                    ...
                    if finish_reason == "length":
                        print(f"   ⚠️  Output truncated — rotating to next model")
                    ...
                    print(f"   ❌ [{attempt_label}] Unexpected error: {e}")
                    if attempt < RETRY_ATTEMPTS:
                        time.sleep(RETRY_BASE_DELAY)

            # inner loop exhausted for current model — rotate
            has_more_models = self._next_model()
            if not has_more_models:
                print(f"   🔄 All models exhausted on key {key_short} — rotating API key")
                has_more_keys = self._next_key()
                if not has_more_keys:
                    print(f"   ⏳ All keys exhausted — waiting {ROTATION_WAIT}s before retry...")
                    time.sleep(ROTATION_WAIT)
                else:
                    print(f"   🔑 Switched to key ...{self.current_key[-6:]}")
            else:
                print(f"   🔀 Switched model -> {self.current_model}")

        raise RuntimeError(
            f"[{label}] All Groq API keys and models exhausted after {tried_this_pass} attempts."
        )


# singleton rotator (created lazily when first needed)
_rotator = None

def get_rotator():
    global _rotator
    if _rotator is None:
        _rotator = GroqRotator(GROQ_API_KEYS, GROQ_MODELS)
    return _rotator
...
def split_decree_into_units(unit):
    """
    Splits a decree that contains articles (الفصل) into:
      - one decree unit (preamble)
      - one unit per article
    ...
    # Pattern to match "الفصل" with optional number or "الوحيد"
    # Examples: الفصل الأول, الفصل 1, الفصل الوحيد
    pattern = r'(الفصل\s+(?:الأول|الثاني|الثالث|الرابع|الخامس|السادس|السابع|الثامن|التاسع|العاشر|الواحد\sوعشرون|الوحيد|\d+))'
    matches = list(re.finditer(pattern, text))
    if not matches:
        return [unit]  # no articles

    # First match position: everything before it is preamble
    first_match = matches[0]
    preamble = text[:first_match.start()].strip()
    decree_unit = unit.copy()
    decree_unit["article_text"] = preamble
    decree_unit["unit_type"] = "DECREE"  # ensure type
    decree_unit["unit_number"] = ""     # decree may not have a number
    ...
    for i, match in enumerate(matches):
        ...
        heading = match.group(1)  # e.g., "الفصل الأول"
        ...
        if number_match:
            ...
            for word, num in ordinal_map.items():
                ...
                article_number = str(i+1)  # fallback
        ...
        article_unit["unit_title"] = heading  # optional
        # Keep the article number also in article_number field
...
def phase1b_locate_units(index, full_text):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # NEW: Split any unit that is a decree and contains articles (الفصل)
    # ─────────────────────────────────────────────────────────────────────────
    final_units = []
    for unit in enriched:
        # Check if this unit should be split
        # Conditions: unit_type is decree-like OR the text contains "الفصل"
        text = unit.get("article_text", "")
        if "الفصل" in text:
            # Try splitting
...
def phase1_extract(pdf_pages, skip_toc_pages=True):
    """
    Phase 1: LLM builds a lightweight index (1a), then Python slices full
    text at anchor positions — zero extra API calls (1b).
    """
...
def _save_incremental(output_path, articles, hierarchical=False):
    # deduped = deduplicate(articles)   # <-- remove this line
    if hierarchical:
...
def run_extraction(input_path, output_path=None, enrich_limit=None,
    ...
    Args:
        input_path:   Path to .pdf or .md file
        output_path:  Output JSON path (default: same dir as input)
        enrich_limit: Only enrich first N articles
        dry_run:      Parse only, skip all API calls
        test_mode:    Enrich 1 article only
        hierarchical: Group articles under parent decrees
    """
    print("\n" + "=" * 60)
    print("🇹🇳 LegaTech Tunisia Extractor v4 — Groq Edition")
    print("=" * 60)

    if not GROQ_API_KEYS:
        raise RuntimeError(
            "No Groq API key found.\n"
            "  Add to your .env:  GROQ_API_KEY=your_key\n"
            "  Or multiple keys:  GROQ_API_KEY_1=...  GROQ_API_KEY_2=..."
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python extractor_groq.py <file> [output.json] "
        ...
        print("  GROQ_API_KEY            single key")
        print("  GROQ_API_KEY_1 / _2 ... multiple keys for rotation")
        sys.exit(1)
    ...
    if results:
        ...
        rotator = get_rotator()
        print(f"  last model:     {rotator.current_model}")